...
@synthesize delegate;
@synthesize recordLayerViewController;
...
- (id)initWithCustomDisplay:(NSString *)nibName {
    if ((self = [super init])) {
        ...
        if (nibName) {
            ...
            recordLayerViewController = sineWave;
        }
...
- (void)dealloc {
    ...
    if (recordLayerViewController) {
        [recordLayerViewController release];
        recordLayerViewController = nil;
    }
...
- (void)postByteData:(NSData *)byteData {
    ...
    NSURL *url = [NSURL URLWithString:@"https://www.google.com/speech-api/v1/recognize?xjerr=1&client=iPhone&lang=zh-CN"];
